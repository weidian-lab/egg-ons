...
module.exports = class ONS {
  ...
  _buildConfig(options, key) {
    const { app, logger, config } = this;
    const distConfig = {
      httpclient: app.httpclient,
      logger,
      instanceName: `${os.hostname()}@${key}`;,
      ...config.default,
      ...options,
    };
    return distConfig;
  ...
  async createConsumer(customKey, options) {
    ...
    const consumer = new Consumer(this._buildConfig(options, customKey));
    ...
    for (const topic of topics) {
      consumer.subscribe(topic, options.subExpression || '*', async msg => {
        if (this.isDuplicateMsg(msg.msgId)) {
        ...
        await ctx.service.ons[customKey].onMsg(msg);
      });
    }
  }
  ...
  async createProducer(key, options) {
    ...
    const producer = new Producer(this._buildConfig(options, key));
