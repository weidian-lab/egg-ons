...
module.exports = class ONS {
  ...
  async createProducer(options) {
    const { app, producerMap, logger, config, topic2Producer, producerRealTopic } = this;
    ...
    for (const topic of topics) {
      ...
      const exist = producerRealTopic.get(newTopic);
      assert(!exist, `producer topic ${newTopic} exist as ${topic}`);
      producerRealTopic.set(newTopic, topic);
    }
  ...
  async send(msg) {
    ...
    const topic = this.producerRealTopic.get(msg.topic);
    assert(!topic, `producer of topic ${topic} not exist`);
    const producer = this.topic2Producer.get(topic);
    assert(producer, `[egg-ons] CANNOT find producer for topic=${topic}`);
