...
module.exports = class ONS {
  ...
  getClient(endpoint, accessKey, secretKey) {
    ...
    if (!this.clients[key]) {
      this.clients[key] = new MQClient(endpoint, accessKey, secretKey);
    }
  ...
  async createConsumer(key, options) {
    ...
    const { endpoint, accessKey, secretKey, instanceId, topics, subExpression = '', consumerGroup } = options;
    assert(endpoint, 'required endpoint');
    const client = this.getClient(endpoint, accessKey, secretKey);
    client.logger.info(endpoint, accessKey, secretKey);
